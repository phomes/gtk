...
{
  return gsk_texture_get_width (GTK_SURFACE_IMAGE (image)->texture);
}

static int
gtk_surface_image_get_height (GtkAbstractImage *image)
{
  return gsk_texture_get_height (GTK_SURFACE_IMAGE (image)->texture);
}
...
gtk_surface_image_snapshot (GtkAbstractImage *image, GtkSnapshot *snapshot)
{
  ...
  gtk_snapshot_append_texture (snapshot,
                               ...
                               &GRAPHENE_RECT_INIT(
                                 0,
                                 0,
                                 gsk_texture_get_width (img->texture) / img->scale,
                                 gsk_texture_get_height (img->texture) / img->scale),
                               "GtkSurfaceImage");
}
...
gtk_surface_image_new (cairo_surface_t *surface)
{
  ...
  image->texture = gsk_texture_new_for_data (cairo_image_surface_get_data (map),
                                  width * scale,
                                  height * scale,
